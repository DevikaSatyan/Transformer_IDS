...
try:
    data = pd.read_csv('car_hacking.csv').astype(np.float32)
...
for class_id in sorted(train_label_counts.keys()):
    print(f"   Class {class_id}: {train_label_counts[class_id]} samples ({train_label_counts[class_id]/len(labels_train)*100:.1f}%)")


val_label_counts = Counter(labels_val)
for class_id in sorted(val_label_counts.keys()):
    print(f"   Class {class_id}: {val_label_counts[class_id]} samples ({val_label_counts[class_id]/len(labels_val)*100:.1f}%)")


test_label_counts = Counter(labels_test)
for class_id in sorted(test_label_counts.keys()):
    print(f"   Class {class_id}: {test_label_counts[class_id]} samples ({test_label_counts[class_id]/len(labels_test)*100:.1f}%)")
